...
=====================================================
Графично приложение за Linux Mint за генериране на сложни двойни пароли
...
def generate_single_password(length: int, use_upper=True, use_lower=True,
    ...
    # Fisher-Yates shuffle с помощта на secrets
...
def load_vault(master_password: str) -> dict:
    ...
    raw = VAULT_PATH.read_bytes()
    if len(raw) <= SALT_SIZE + 3:
        raise ValueError("Файлът на трезора е повреден или празен.")
    ...
    # Миграция: превръщане на единични обекти в списъци
...
def save_vault(data: dict, master_password: str) -> int:
    ...
    # Atomic write за предотвратяване от повреждане при неочакван срив
...
class LoginDialog(simpledialog.Dialog):
    ...
    def body(self, master):
        ...
        ttk.Label(master, text="Парола:").grid(row=1, column=0, sticky="e", padx=4)
        self.entry1 = ttk.Entry(master, show="•", width=30)
        self.entry1.grid(row=1, column=1, pady=4, padx=4)

        if self.confirm:
            ttk.Label(master, text="Повтори:").grid(row=2, column=0, sticky="e", padx=4)
...
class PasswordManagerApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Мениджър на двойни пароли")
    ...
    def _build_ui(self):
        ...
        notebook.add(self.gen_tab, text="Генератор на Двойни Пароли")
        ...
        settings_menu.add_command(label="Изход", command=self.destroy)
    ...
    def _build_generator_tab(self):
        ...
        length_frame = ttk.LabelFrame(frame, text="Дължина на двете пароли (от 16 до 24 символа)")
        length_frame.pack(fill="x", **pad)

        ttk.Label(length_frame, text="Парола 1:").grid(row=0, column=0, padx=8, pady=6)
        ...
        ttk.Label(length_frame, text="Парола 2:").grid(row=1, column=0, padx=8, pady=6)
        ...
        ttk.Checkbutton(opts_frame, text="Разреши еднакви дължини (само случайно)",
                        variable=self.allow_equal).grid(row=3, column=0, columnspan=2, sticky="w", padx=8, pady=4)

        result_frame = ttk.LabelFrame(frame, text="Генерирана двойна парола")
        ...
        self.info_var = tk.StringVar(value="Дължина: - | Сила: -")
        ttk.Label(frame, textvariable=self.info_var, font=("Sans", 9, "bold")).pack(anchor="w", padx=12, pady=4)

        btns = ttk.Frame(frame)
        btns.pack(fill="x", pady=8)
        ttk.Button(btns, text="Генерирай двойна парола", command=self.on_generate).pack(side="left", padx=8)
    ...
    def _build_vault_tab(self):
        ...
        # Панел за търсене и действия
        top_bar = ttk.Frame(frame)
        top_bar.pack(fill="x", **pad)

        ttk.Label(top_bar, text="Търсене:").pack(side="left", padx=(4, 2))
        ...
        ttk.Button(top_bar, text="Добави нов", command=self.add_manual_entry).pack(side="left", padx=4)
        ttk.Button(top_bar, text="Редактирай", command=self.edit_selected_entry).pack(side="left", padx=4)
        ttk.Button(top_bar, text="Изтрий", command=self.delete_selected_entry).pack(side="left", padx=4)

        columns = ("service", "username")
        self.tree = ttk.Treeview(frame, columns=columns, show="headings", selectmode="browse")
        self.tree.heading("service", text="Услуга / Сайт")
        ...
        ttk.Button(detail_frame, text="Копирай", command=self.copy_detail_password).grid(row=0, column=3, padx=4)
    ...
    # ---------------- Помощна логика за сигурност ----------------

    def _safe_copy_to_clipboard(self, text: str):
        """Копира текст и го изчиства автоматично след определено време."""
        ...
        messagebox.showinfo("Копирано", "Паролата е копирана! Клипбордът ще се изчисти след 30 сек.")
    ...
    def _unlock_or_create_vault(self):
        ...
        prompt = ("Файлът с пароли съществува.\nВъведи главната си парола:"
                  if vault_exists else
                  "Няма намерен съществуващ трезор.\nЗадади главна парола:")
        ...
        if not vault_exists:
            ...
            messagebox.showinfo(
                "Успех",
                f"Трезорът е създаден успешно!\nКалибровка: scrypt N=2^{n_pow} (~{mem_mb:.0f} MB RAM)."
            )
    ...
    def _update_strength(self):
        ...
        self.info_var.set(f"Формат: {l1}-{l2} ({total_len + 1} символа) | Сила: {strength_label(bits)} (~{bits:.0f} бита ентропия)")
    ...
    def edit_selected_entry(self):
        sel = self.tree.selection()
        if not sel:
            messagebox.showwarning("Няма избор", "Избери запис за редакция.")
            return
    ...
    def _open_entry_dialog(self, service_name="", username="", prefilled_password="", notes="", edit_target=None):
        ...
        top.title("Редактирай запис" if edit_target else "Нов запис")
        ...
        def do_save():
            ...
            # При редакция изтриваме стария запис първо
            if edit_target:
            ...
            self.refresh_vault_list()
            top.destroy()
            messagebox.showinfo("Готово", f"Записът за '{new_service}' е запазен.")
    ...
    def delete_selected_entry(self):
        ...
        msg = f"Наистина ли да изтрия акаунт '{username}' за '{service}'?" if username else f"Наистина ли да изтрия този запис за '{service}'?"

        if messagebox.askyesno("Потвърждение", msg):
