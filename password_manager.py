...
=============================
Проста конзолна програма за Linux (тествана за Linux Mint), която:
  1. Генерира сложни, случайни пароли.
  2. Съхранява паролите криптирано на диска (защитени с главна парола).

Съхранението използва:
  - scrypt (RFC 7914) за извеждане на ключ от главната парола — за разлика от
    PBKDF2, scrypt е "memory-hard": всеки опит изисква не само процесорно
    време, но и десетки/стотици MB RAM, което прави GPU/ASIC атаки много
    по-скъпи (същият подход, който LUKS2 и повечето модерни password
    manager-и ползват чрез Argon2id). Броят "тежест" се калибрира спрямо
    компютъра при създаване на трезора, целейки ~1 секунда на опит.
    scrypt е вграден в стандартната библиотека на Python (hashlib) —
    не изисква инсталиране на никакъв допълнителен пакет.
...
е скриптът — това го прави преносим: копирай папката (скрипт + .enc
файл) на флашка/външен диск и ще работи на всеки компютър с Python.
Файлът винаги е криптиран на диска. Данните се дешифрират само в
паметта, докато програмата работи, и то само след като въведеш
вярната главна парола.
...
SCRYPT_MAX_N_POW = 20        # горна граница (2^20 * 128 * r * p ≈ 1 GB) — предпазна мярка
...
def generate_password(length: int = 16,
    ...
    """Генерира криптографски сигурен случаен паролa."""
    ambiguous = "Il1O0"
    ...
    # Гарантираме поне по един символ от всеки избран тип
    password_chars = [secrets.choice(pool) for pool in pools]
    password_chars += [secrets.choice(all_chars) for _ in range(length - len(pools))]

    # Разбъркваме сигурно (Fisher-Yates със secrets)
...
def derive_key(master_password: str, salt: bytes, n_pow: int, r: int, p: int) -> bytes:
    ...
    maxmem = mem_needed * 2 + (16 * 1024 * 1024)  # запас, за да не гърми на лимита
...
def calibrate_scrypt(target_seconds: float = TARGET_KDF_SECONDS,
    ...
    """Удвоява N (стъпка на scrypt), докато времето за извеждане на ключа
    достигне ~target_seconds, спазвайки горна граница на паметта. Връща n_pow
    (действителното N е 2**n_pow)."""
    salt = os.urandom(SALT_SIZE)
    n_pow = 14  # старт: 16 MB памет, бързо
    while True:
        n = 2 ** n_pow
...
def load_vault(master_password: str) -> dict:
    """Зарежда и дешифрира vault-а. Ако не съществува, връща празен речник."""
    if not VAULT_PATH.exists():
        return {}
...
def save_vault(data: dict, master_password: str) -> None:
    """Криптира и записва vault-а на диска. При нов файл калибрира параметрите
    на scrypt спрямо текущия компютър; при съществуващ файл преизползва вече
    запазените сол/параметри, за да не се променят при всеки запис."""
    header = _read_header()
    if header is not None:
        ...
        print(f"(Калибровка: scrypt N=2^{n_pow}, r={r}, p={p} "
              f"(~{mem_mb:.0f} MB памет на опит, ~{TARGET_KDF_SECONDS:.1f} сек на този компютър))")
    ...
    os.chmod(VAULT_PATH, 0o600)  # само собственикът може да чете/пише
...
def change_master_password_flow():
    ...
    # Пресъздаваме файла с нова сол и нов ключ
    if VAULT_PATH.exists():
        VAULT_PATH.unlink()
    save_vault(vault, new_master)
